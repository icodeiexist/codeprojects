...
class major:
    ...
    codes = [('^r'),('^t','^d'),('^n','^ñ'),('^m',),('^c[^aeiouh]','^ca','^co','^cu','^k','^q'),('^l','^y'),('^s','^z','^ce','^ci'),('^j','^f','^ge','^gi'),('^ch','^ga','^go','^gu','^sh','^g[^aeiou]'),('^p','^v','^b')]
   def encode(self,word):
      matches = []
      h_mx = hyphenator('es_MX')
      for idx,code in enumerate(self.codes):
         for pattern in code:
            for syllable in h_mx.syllables(word)
                itera = re.finditer(pattern,syllable)
                if itera:
                   for match in itera:
